    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sessions.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_agenda') ? 'is-invalid' : '' }}" name="nama_agenda_id" id="nama_agenda_id" required>
                    @foreach($nama_agendas as $id => $entry)
                        <option value="{{ $id }}" {{ old('nama_agenda_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_sesi') ? 'is-invalid' : '' }}" type="text" name="nama_sesi" id="nama_sesi" value="{{ old('nama_sesi', '') }}" required>
