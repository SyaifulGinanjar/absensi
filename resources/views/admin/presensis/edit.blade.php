...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.presensi.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.presensis.update", [$presensi->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_sesi') ? 'is-invalid' : '' }}" name="nama_sesi_id" id="nama_sesi_id">
                    @foreach($nama_sesis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nama_sesi_id') ? old('nama_sesi_id') : $presensi->nama_sesi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_peserta') ? 'is-invalid' : '' }}" name="nama_peserta_id" id="nama_peserta_id">
                    @foreach($nama_pesertas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nama_peserta_id') ? old('nama_peserta_id') : $presensi->nama_peserta->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $presensi->type) }}">
                @if($errors->has('type'))
                    <div class="invalid-feedback">
                        {{ $errors->first('type') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $presensi->status) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('refer_to') ? 'is-invalid' : '' }}" type="text" name="refer_to" id="refer_to" value="{{ old('refer_to', $presensi->refer_to) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('waktu') ? 'is-invalid' : '' }}" type="text" name="waktu" id="waktu" value="{{ old('waktu', $presensi->waktu) }}">
                @if($errors->has('waktu'))
                    <div class="invalid-feedback">
                        {{ $errors->first('waktu') }}
                    </div>
